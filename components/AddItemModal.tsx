    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-4">
      <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-purple-600 to-violet-600 px-6 py-5 flex items-center justify-between">
          <h2 className="text-white font-bold text-lg">הוסף מוצר</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          {/* Product name */}
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">שם המוצר</label>
            <div className="relative">
              <input
                type="text" value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder="לדוגמה: חלב, עגבניות, עוף..."
                required autoFocus
              ...
              <div className="absolute left-3 top-1/2 -translate-y-1/2">
                {aiCategorizing && <Loader2 className="w-4 h-4 text-purple-400 animate-spin" />}
                {!aiCategorizing && name.length >= 2 && <Sparkles className="w-4 h-4 text-purple-400" />}
              </div>
            </div>
            {aiCategorizing && (
              <p className="text-xs text-purple-400 mt-1 flex items-center gap-1">
                <Sparkles className="w-3 h-3" /> AI מזהה קטגוריה...
              </p>
            )}
            {!aiCategorizing && name.length >= 2 && (
              <p className="text-xs text-purple-500 mt-1 flex items-center gap-1">
                <Sparkles className="w-3 h-3" /> קטגוריה תוגדר אוטומטית
              </p>
            )}
          </div>
          ...
          <button type="submit" disabled={loading || !name.trim()}
            ...
            {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <><Plus className="w-5 h-5" strokeWidth={2.5} />הוסף לרשימה</>}
          </button>
        </form>
      </div>
    </div>
