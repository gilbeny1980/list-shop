...
import { AdminSession, BotOrder, BotProduct, WaSession } from "@/types/shopBot";
...
export async function getWaSession(phone: string): Promise<WaSession> {
  return (await kvGet<WaSession>(`shopbot:wa:${phone}`)) || {};
}
export async function setWaSession(phone: string, session: WaSession): Promise<void> {
